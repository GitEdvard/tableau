...
select distinct prt.protocol_type, prj.name as project_name, pt.displayname,
                auv.udfvalue as pooling_artifact, suv.udfvalue pooling_sample,
                ...
                cast(substring(suv.udfvalue from '([0-9\.]+).*') as int)
                else
                ...
                cast(substring(auv.udfvalue from '([0-9\.]+).*') as int)
                end as lib_per_pool
...
group by prt.protocol_type, prj.name, pt.typename, pt.displayname, auv.udfvalue, suv.udfvalue
